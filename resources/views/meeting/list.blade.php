...
@section('info-page')
    ...
    <h5 class="font-weight-bolder mb-0 text-capitalize">Meeting List</h5>
@endsection
@section('content')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#table-data').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "{{ env('URL_API') }}/api/v1/meeting/list",
                    "type": "GET",
                ...
                "columns": [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return `
            <a class="btn btn-primary w-100 open-meeting-dialog" data-guid="${row.meeting_id}" role="button" style="text-decoration: none;" data-bs-toggle="tooltip">Join Meeting</a>
        `;
                        },
                        "orderable": false,
                        "searchable": false
            ...
            }), $("div.head-label").html('<h5 class="card-title mb-0">Meeting List</h5>');
            ...
            $(document).on("click", ".open-meeting-dialog", function () {
                var meetingId = $(this).data('guid'); // Ambil meeting_id dari atribut data-guid
                var userId = '{{ $id }}'; // Ambil id dari session
                var userEmail = '{{ $email }}'; // Ambil email dari session

                if (meetingId) {
                    // Buat URL dengan id dan email
                    var zoomRedirectUrl = `http://localhost:8004/api/v1/zoom/join/` + meetingId + `?user_id=` + userId + `&user_email=` + userEmail;
                    window.location.href = zoomRedirectUrl; // Redirect ke endpoint dengan meeting_id, user_id, dan user_email
                } else {
